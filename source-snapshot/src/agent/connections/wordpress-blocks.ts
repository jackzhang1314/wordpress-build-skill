...
export const wpLink=z.string().max(2000).refine(value=>{try{return /^(https?:|mailto:|tel:)$/.test(new URL(value).protocol);}catch{return /^#[A-Za-z0-9_-]+$/.test(value);}},'链接仅支持HTTP(S)、邮件、电话或页内锚点。');
const leaf=z.discriminatedUnion('type',[
 ...
 z.object({type:z.literal('catalog'),postType:z.enum(['oct_product','oct_case']),perPage:z.number().int().min(1).max(24).default(9)}),
]);
...
export function serializeWpBlocks(raw:WpBlocks,media:ReadonlyMap<number,string>):string{
 ...
 const render=(node:WpBlocks[number]):string=>{
  switch(node.type){
   ...
   case 'button':return block('buttons',undefined,`<div class="wp-block-buttons">${block('button',undefined,`<div class="wp-block-button"><a class="wp-block-button__link wp-element-button" href="${escapeHtml(node.url)}">${escapeHtml(node.text)}</a></div>`)}</div>`);
