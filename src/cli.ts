...
async function main(): Promise<unknown> {
  const args = parseArgs({ allowPositionals: true, strict: true, options: { plan: { type: 'string' }, task: { type: 'string', default: '.wordpress-builder' }, id: { type: 'string' }, type: { type: 'string' }, publish: { type: 'boolean', default: false }, help: { type: 'boolean' } } });
  const command = args.positionals[0];
  if (args.values.help || !command) return {
    commands: ['doctor', 'read-page --id ID', 'build --plan FILE [--publish]', 'status', 'template-parts', 'navigation-plan --plan FILE', 'navigation-apply --id PLAN_HASH', 'page-edit-plan --plan FILE', 'page-edit-apply --id PLAN_HASH', 'upload-media --plan FILE', 'content-type --type TYPE', 'read-content --type TYPE --id ID', 'content-plan --plan FILE', 'content-apply --id PLAN_HASH', 'import-plan --plan FILE', 'import-apply --id PLAN_HASH', 'templates', 'read-template --id THEME//SLUG', 'single-template-plan --plan FILE', 'single-template-apply --id PLAN_HASH'],
  ...
  switch (command) {
    ...
    case 'read-page': { const page = await readPage(client, z.coerce.number().int().positive().parse(args.values.id)); await journal.save(`read-page-${page.id}.json`, page); return { ...page, version: pageVersion(page), rawComplete: true }; }
    case 'build': return buildSite(client, journal, sitePlanSchema.parse(await readJson(z.string().min(1).parse(args.values.plan))), args.values.publish ? 'publish' : 'draft');
