...
await new Promise((accept, reject) => { probe.once('error', reject); probe.listen(9466, '127.0.0.1', accept); });
...
const run = await mkdtemp(resolve('.lab/hongda-clean-'));
...
const delivery=JSON.parse(await readFile('.lab/hongda-package-latest.json','utf8'));
await cp(delivery.directory,join(run,'delivery'),{recursive:true});
const steps = [
  ...['advanced-custom-fields/acf.php', 'fluentform/fluentform.php', 'autodescription/autodescription.php'].map(pluginPath => ({ step: 'activatePlugin', pluginPath })),
  {step:'installPlugin',pluginData:{resource:'vfs',path:'/delivery/site-model.zip'},options:{activate:true}},
  {step:'installTheme',themeData:{resource:'vfs',path:'/delivery/site-reference.zip'},options:{activate:true}},
...
steps.push({step:'runPHP',code: `<?php require '/wordpress/wp-load.php'; $expected=json_decode(file_get_contents('/delivery/manifest.json'),true); foreach($expected['files'] as $f){$base=str_starts_with($f['path'],'site-reference/')?get_theme_root():WP_PLUGIN_DIR; if(hash_file('sha256',$base.'/'.$f['path'])!==$f['sha256'])throw new RuntimeException('Installed file mismatch');} file_put_contents('/artifacts/package-install.json',wp_json_encode(['filesVerified'=>count($expected['files']),'theme'=>get_stylesheet(),'source'=>'WordPress ZIP installer']));`});
const validation = validateBlueprint(blueprint);
if (!validation.valid) throw new Error(JSON.stringify(validation.errors));
await writeFile(join(run, 'blueprint.json'), JSON.stringify(blueprint, null, 2));
await writeFile(join(run, 'loopback.mjs'), "import {Server} from 'node:net'; const original=Server.prototype.listen; Server.prototype.listen=function(...args){if(args[0]===9466&&(args[1]===undefined||typeof args[1]==='function'))return Reflect.apply(original,this,[9466,'127.0.0.1',...args.slice(1)]);return Reflect.apply(original,this,args);};");
...
const args = ['--experimental-wasm-jspi','--import='+join(run,'loopback.mjs'),resolve('node_modules/.bin/wp-playground-cli'),'server','--wp=7.1.1','--php=8.3','--port=9466','--workers=6','--blueprint='+join(run,'blueprint.json'),
  '--mount-dir',join(run,'source'),'/source',
  '--mount-dir',join(run,'delivery'),'/delivery',
...
console.log('HONGDA clean ZIP acceptance at http://127.0.0.1:9466; private artifacts:', artifacts);
...
try {
 await waitForWordPress({url:'http://127.0.0.1:9466',artifact:join(artifacts,'site.json'),php:'8.3',wordpress:'7.1.1',isRunning:()=>running});
 await writeFile('.lab/hongda-clean-latest.json', JSON.stringify({ run, artifacts, url: 'http://127.0.0.1:9466' }));
