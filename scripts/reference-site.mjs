...
await new Promise((accept, reject) => { probe.once('error', reject); probe.listen(9463, '127.0.0.1', accept); });
...
const run = await mkdtemp(resolve('.lab/reference-'));
...
await cp('.agents/skills/wordpress-builder/assets/php-reference', join(run, 'source'), { recursive: true });
...
const steps = [
  ...
  { step: 'runPHP', code: await readFile('scripts/fixtures/new-site/setup.php', 'utf8') },
];
...
await writeFile(join(run, 'blueprint.json'), JSON.stringify(blueprint, null, 2));
await writeFile(join(run, 'loopback.mjs'), "import {Server} from 'node:net'; const original=Server.prototype.listen; Server.prototype.listen=function(...args){if(args[0]===9463&&(args[1]===undefined||typeof args[1]==='function'))return Reflect.apply(original,this,[9463,'127.0.0.1',...args.slice(1)]);return Reflect.apply(original,this,args);};");
...
await writeFile('.lab/reference-latest.json', JSON.stringify({ run, artifacts, url: 'http://127.0.0.1:9463' }));
const args = ['--experimental-wasm-jspi','--import='+join(run,'loopback.mjs'),resolve('node_modules/.bin/wp-playground-cli'),'server','--wp=7.1.1','--php=8.3','--port=9463','--workers=6','--blueprint='+join(run,'blueprint.json'),
...
console.log('Fresh reference at http://127.0.0.1:9463; private artifacts:', artifacts);
