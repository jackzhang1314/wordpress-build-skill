...
> 状态：已实施（2026-09-22）。中央入口为 `harness/cli.mjs`，以 `project.json` 驱动经典 PHP + ACF 项目。
...
- [x] 一条命令完成部署（含前置检查+备份+验证+失败文件回滚）
- [x] 一条命令完成全部质量关卡
- [x] 新站初始化一条 `init` 命令生成经典主题骨架
- [ ] 文档 ≤ 5 个核心 reference（经典 ACF 入口和 19 号真源已建立；旧文档仍在维护期）
- [x] 全部新操作有测试覆盖；当前 84/84 tests 通过
- [x] 72+ 现有测试不回归（当前套件 84 个全部通过）
