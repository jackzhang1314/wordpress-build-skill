...
function destination(string $key): string { $p=get_post((int)get_theme_mod($key,0)); return $p instanceof \WP_Post && $p->post_type==='page' && $p->post_status==='publish'?get_permalink($p):home_url('/'); }
...
function media(int $id, bool $hero=false): void {
    ...
    echo '<div class="media-placeholder"><span>HONGDA</span><small>Equipment image coming soon</small></div>';
}
