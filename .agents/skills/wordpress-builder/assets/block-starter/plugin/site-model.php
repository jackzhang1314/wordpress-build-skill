 ...
 * Description: B2B starter equipment and industry content model. Requires ACF for field editing.
 * Version: 1.1.0
...
function register_model(): void {
    register_post_type('hd_product', ['labels'=>['name'=>'Equipment','singular_name'=>'Machine'], 'description'=>'Compare equipment by application, operating weight and technical specifications. Review each model and request confirmation of the configuration for your working conditions.','public'=>true,'show_in_rest'=>true,'has_archive'=>'equipment','rewrite'=>['slug'=>'equipment/model','with_front'=>false], 'menu_icon'=>'dashicons-admin-tools','supports'=>['title','editor','excerpt','thumbnail','revisions']]);
    ...
    register_post_type('hd_solution', ['labels'=>['name'=>'Industries','singular_name'=>'Industry solution'],'description'=>'Start with the conditions on your site. Explore the equipment questions that matter for each application.','public'=>true,'show_in_rest'=>true,'has_archive'=>'industry','rewrite'=>['slug'=>'industry','with_front'=>false],'menu_icon'=>'dashicons-building','supports'=>['title','editor','excerpt','thumbnail','revisions']]);
}
...
add_action('acf/init', static function (): void {
    ...
    foreach ([['model','Model','text'],['weight','Operating weight (tonnes)','number'],['specifications','Technical specifications','textarea'],['working_range','Working range','textarea'],['dimensions','Dimensions','textarea']] as [$name,$label,$type]) {
        $fields[]=['key'=>'field_hd_'.$name,'name'=>$name,'label'=>$label,'type'=>$type,'allow_in_bindings'=>1,'instructions'=>$type==='textarea'?'One specification per line: Label | Value':'','min'=>0,'step'=>'any'];
    }
