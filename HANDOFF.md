...
Only the selected route template or a selected part can be patched, using one unique `find`, one validated `replace` and live `verifyText`. A theme source becomes a WordPress custom override rather than editing theme files; rollback restores custom content or deletes a newly created override.

WordPress Builder `2.23.0` added the first supported Block/FSE write path:
...
tests: 246 / 246 pass
build: pass
clean clone of v2.24.0: pending until tag publication
```
