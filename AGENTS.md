...
- 用户环境统一入口是 `node harness/cli.mjs bootstrap [--fix]`；Hostinger CLI 入口是 `hostinger setup [--install] [--connect]`；项目 SSH 入口是 `--project <dir> ssh setup`。不要引导用户编辑 SSH 字段，除非向导失败并明确要求 override。
