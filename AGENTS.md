...
- 任意 WordPress 站（Classic、Block/FSE、Elementor、Divi、Beaver Builder、WPBakery、Bricks、Oxygen、混合实现）都可被 Builder 管理。内容模型、页面、文章、导航、ACF/CPT 操作是主题无关的；渲染系统只决定使用哪个 adapter。给老站新增 Builder-managed 页面时，先安装独立的 `wordpress-builder-core` 插件，不转换、不覆盖原 Elementor/Divi/自定义编辑器页面。
