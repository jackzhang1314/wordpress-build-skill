...
- Hostinger SSH key 按 hosting account/SSH user 管理：新项目默认用 `~/.ssh/hostinger-<user>_ed25519`，不复制旧项目 key。首次可用 `ssh setup --account-key --install-key` 通过已授权 CLI 的 Files + temporary Cron bootstrap；失败才降级 `--copy-key --open` 的 hPanel handoff。不要把这个说成 Hostinger 登录账号级，也不要在不同 hosting order/user 之间冒充通用。
